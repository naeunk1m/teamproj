...
<script src="https://kit.fontawesome.com/daddbc6c0e.js"
...
<script>
	$(document).ready(function() {
		$('#btn2').click(function() {
			$.ajax({
				url : './Member_SH/phonedpcheck.jsp',
				data : {
					'phone' : $('#phone').val()
				},
				success : function(rdata) {
					if (rdata.trim() == '1') {
						$('#phone2').val(1);
						$('#phonediv').html('이미 가입되어 있는 번호입니다.');
					} else {
						$('#phone2').val(0);
						$('#phonediv').html("사용 가능한 번호입니다.");
					}
				}
			});
		});
...
<script>
	$(document).ready(function() {
		$('#fr').submit(function() {
			...
			if ($('#c2').is(':checked') == false) {
				alert('이용약관[2]에 동의해주세요.');
				return false;
			}
		});
	});
...
<body>
	...
	<section>
		<h1>회원가입</h1>
		<form action="./MemberInsertAction.me" method="post" id="fr">
			<div class="textForm">
				<h4>아이디</h4>
				<input type="text" name="u_id" id="u_id" placeholder="5~20자의 영문 소문자, 숫자와 특수기호(_),(-)만 사용가능합니다."> <input
					type="hidden" name="u_id2" id="u_id2" value="0"> <input
					type="button" value="아이디 중복체크" id="btn1" style="float:right; ">
				<div id="iddiv"></div>
			</div>

			<div class="textForm">
				<h4>비밀번호</h4>
				<input name="u_pw" type="password" class="u_pw"
					placeholder="비밀번호를 입력해주세요.">
			</div>
			<div class="textForm">
				<h4>비밀번호 확인</h4>
				<input name="loginPwConfirm" type="password" class="u_pw"
					placeholder="비밀번호를 한 번 더 입력해주세요.">

			</div>
			<div class="textForm">
				<h4>이름</h4>
				<input name="u_nm" type="text" class="u_nm"
			...
			<div class="textForm">
				<h4>이메일</h4>
				<input name="email1" type="text" class="email" placeholder="이메일"">
				@ <input name="email2" class="email2" list="domains"
					placeholder="주소 선택">
				<datalist id="domains">
			...
			<div class="textForm">
				<h4>전화번호</h4>
				<input type="text" name="phone" id="phone" class="cellphoneNo"
					placeholder="(-)를 제외하고 입력해주세요. ">
				<!-- 			<input type="tel" name="phone1" class="cellphoneNo"  placeholder="전화번호 ">- -->
				<!--             <input type="tel" name="phone2" class="cellphoneNo"  placeholder="전화번호">- -->
				<!--             <input type="tel" name="phone3" class="cellphoneNo"  placeholder="전화번호"> -->
				<input type="hidden" name="phone2" id="phone2" value="0"> <input
					type="button" value="확인" id="btn2" >
				<div id="phonediv"></div>
			</div>
			...
			<div class="textForm">
				...
				<div class="textFormgenre">
					...
					<input type="checkbox" name="favorit" value="로맨틱코미디" style="zoom:1.7;">로맨틱 코미디 <input
						...
						<input type="checkbox" name="favorit" value="코미디" style="zoom:1.7;">코미디
				</div>
			</div>
			<div class="textForm">
				<h4>뉴스레터 및 (장르)관련 소식을 메일로 받으시겠습니까 ?</h4>
				<br> <input type="radio" name="mailing" value="동의">동의 <input
					type="radio" name="mailing" value="동의안함">동의안함
			</div>
			<br>
			<div class="read">
				<h2 style="margin-left:55px; margin-bottom:20px;">✔ 이용약관[1]</h2>
				<textarea readonly="readonly" rows="9" cols="100" style="width:590px; margin-left:25px;">
        제 1 조 (목적)

        제 1 장 총칙

        제 1 조 (목적)
        본 약관은 시네마윌 홈페이지(이하 "당 사이트")에서 제공하는 모든 서비스(이하 "서비스")의 이용조건 및 절차, 이용자와 당 사이트의 권리, 의무, 책임사항과 기타 필요한 사항을 규정함을 목적으로 합니다.
        ...
        (시행일) 본 약관은 2018년 8월 1일부터 시행됩니다. 개정된 약관의 적용일자 이전 이용자 또는 회원은 개정된 이용약관의 적용을 받습니다.
				...
				<h3>
					<input type="checkbox" name="c1" id="c1" value="c1" style="margin-left:430px; margin-bottom: 10px; ">이용약관
					안내(필수)
				</h3>
				
				<h2 style="margin-left:55px; margin-bottom:20px;">✔ 이용약관[2]</h2>
				<textarea readonly="readonly" rows="9" cols="100" style="width:590px; margin-left:50px; margin-left:25px;">
        제 1 조 (목적)

        이 약관은 시네마윌 주식회사 ("회사" 또는 "CinemaWill")가 제공하는 시네마윌 및 시네마윌 관련 제반 서비스의 이용과 관련하여 회사와 회원과의 권리, 의무 및 책임사항, 기타 필요한 사항을 규정함을 목적으로 합니다.
        ...
        * 수탁업체
        ...
        · 개인정보 보유 및 이용 기간 : 행정자치부에서는 이미 보유하고 있는 개인정보이기 때문에 별도로 저장하지 않음
				...
				<h3>
					<input type="checkbox" name="c2" id="c2" value="c2" style="margin-left:250px; margin-bottom: 10px; ">개인정보 수집
					및 이용에 대한 안내(필수)
				</h3>
				
				<h2 style="margin-left:55px; margin-bottom:20px;">✔ 이용약관[3]</h2>
				<textarea readonly="readonly" rows="9" cols="100" style="width:590px; margin-left:50px; margin-left:25px;">
        제 1 조 (목적)

        시네마윌에서 제공하는 이벤트/혜택 등 다양한 정보를 휴대전화(앱 알림 또는 문자), 이메일로 받아보실 수 있습니다. 일부 서비스(별도 회원 체계로 운영하거나 시네마윌 가입 이후 추가 가입하여 이용하는 서비스 등)의 경우, 개별 서비스에 대해 별도 수신 동의를 받을 수 있으며, 이때에도 수신 동의에 대해 별도로 안내하고 동의를 받습니다.


        </textarea>
				<h3>
					<input type="checkbox" name="c3" id="c3" value="c3" style="margin-left:340px;">프로모션 정보
					수신 동의(선택)
				</h3>
